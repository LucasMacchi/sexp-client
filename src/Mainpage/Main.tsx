...
export default function Main() {
  ...
  useEffect(() => {
    if (editMode && exp) {
      ...
      setModDateFac(
        exp.fecha_facturacion ? dateReturner(exp.fecha_facturacion, true) : "",
      );
      setTesoreria(
  ...
  const empresaReturner = (id: number): string => {
    ...
    empresas.forEach((e) => {
      if (e.empresa_id === id)
        name = e.nombre + " - " + servicioReturner(e.servicio_id);
    });
    return name;
  };
  ...
  const editExp = async () => {
    if (confirm("¿Quiere modificar el expediente?") && exp?.exp_id) {
      const data: IModExp = {
        ...
        invitacion:
          modInvitacion !== exp.invitacion ? modInvitacion : exp.invitacion,
        orden_compra:
          modOrdenCompra !== exp.orden_compra
            ? modOrdenCompra
            : exp.orden_compra,
        fecha_facturacion:
          modDateFac !== exp.fecha_facturacion ? modDateFac : "",
        fecha_tesoreria:
          modTesoreria !== exp.fecha_tesoreria ? modTesoreria : "",
      };
      ...
      return (
        <div className={deleteColumn("div-filter")}>
          ...
          <div>
            ...
            <select
              value={filter.estado}
              onChange={(e) => handleFilter(parseInt(e.target.value), "estado")}
            >
              <option value={0}>---</option>
              {estados.map((e) => (
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                handleFilter(parseInt(e.target.value), "empresa")
              }
            >
              <option value={0}>---</option>
              {empresas.map((e) => (
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => handleFilter(e.target.value, "periodo")}
            >
              <option value={""}>---</option>
              {meses.map((e) => (
          ...
          <div>
            ...
            <input
              type="date"
              value={filter.end}
              onChange={(e) => handleFilter(e.target.value, "end")}
            />
          </div>
          <div>
            <h6 className="filter-title">Guardar filtro</h6>
            <button onClick={() => saveFilter()} className="filter-btn-save">
              Guardar
            </button>
          </div>
        </div>
    ...
    if (exp_id && exp) {
      return (
        <div className="div-exp-detail">
          ...
          <h4 className="exp-data-h">
            {dateReturner(exp.fecha_presentacion, false)}
          </h4>
          <hr color="#3399ff" />
          ...
          {editMode ? (
            ...
            <h4 className="exp-data-h">
              {dateReturner(exp.fecha_ult_mod, false)}
            </h4>
          )}
          <hr color="#3399ff" />
          ...
          {editMode ? (
            <input
              type="date"
              value={modDateFac}
              onChange={(e) => setModDateFac(e.target.value)}
            />
          ) : (
            <h4 className="exp-data-h">
              {exp.fecha_facturacion
                ? dateReturner(exp.fecha_facturacion, false)
                : "NaN"}
            </h4>
          )}
          <hr color="#3399ff" />
          ...
          {editMode ? (
            <input
              type="date"
              value={modTesoreria}
              onChange={(e) => setTesoreria(e.target.value)}
            />
          ) : (
            <h4 className="exp-data-h">
              {exp.fecha_tesoreria
                ? dateReturner(exp.fecha_tesoreria, false)
                : "NaN"}
            </h4>
          )}
          <hr color="#3399ff" />
          ...
          {editMode ? (
            <div className="input-div-register">
              ...
              <input
                type="checkbox"
                checked={modInvitacion}
                onChange={(e) => setModInvitacion(e.target.checked)}
              />
            </div>
          ) : (
            <h4 className="exp-data-h">
              Invitacion: {exp.invitacion ? "Si" : "No"}
            </h4>
          )}
          <hr color="#3399ff" />
          {editMode ? (
            <div className="input-div-register">
              ...
              <input
                type="checkbox"
                checked={modOrdenCompra}
                onChange={(e) => setModOrdenCompra(e.target.checked)}
              />
            </div>
          ) : (
            <h4 className="exp-data-h">
              Orden de Compra: {exp.orden_compra ? "Si" : "No"}
            </h4>
          )}
          <hr color="#3399ff" />
          ...
          {editMode ? (
            ...
            <p className="description-exp">{exp.descripcion}</p>
          )}

          <div className="div-exp-btn">
            <button className="btn-exp" onClick={() => setExpId(0)}>
              Salir
            </button>
            <button
              className={editMode ? "btn-exp" : "delete-column"}
              onClick={() => editExp()}
            >
              Modificar
            </button>
  ...
  const loginChecker = () => {
    ...
    if (token) {
      ...
      return (
        <div>
          <button className="btn-big" onClick={() => navigator('/login')}>Acceder al Sistema</button>
        </div>
      );
    }
  };
