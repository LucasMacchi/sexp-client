...
export const useUserStore = create<IUserStore>((set) => ({
    ...
    modCredential: async ( empresaId: number, userId?: number) => {
        try {
            if(userId){
                await axios.post(SERVER+'/user/credential/'+userId+'/'+empresaId,{}, authReturner())
                alert('Credenciales modificadas.')
            }
            else{
                await axios.delete(SERVER+'/user/credential/'+empresaId, authReturner())
    ...
    getAllUsers: async () => {
        try {
            const users: IUser[] = (await axios.get(SERVER+'/user/all', authReturner())).data
            set({sysUsers: users})
        } catch (error) {
